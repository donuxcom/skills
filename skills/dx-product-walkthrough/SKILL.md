---
name: dx-product-walkthrough
description: Use when a repository needs product onboarding covering capabilities, modules, user journeys, roles, and integrations; not for implementation architecture alone.
license: Apache-2.0
...
Create an evidence-backed, start-to-finish repository walkthrough through the lens of product capabilities and user-facing behavior. This is a teaching document, not a file inventory or unsupported critique.
...
Do not use for:
- Trace every internal function call for an engineer.
- Audit the component library’s visual consistency.
- Document only entity relationships and state transitions.
...
1. Confirm the repository root, requested audience, scope, excluded areas, format, and output path. Default to `product-walkthrough.html`; use `product-walkthrough.md` only when Markdown is explicitly requested. Check whether the selected output already exists. **Complete when:** boundaries, format, and overwrite status are explicit.
2. Build a bounded inventory without opening excluded or sensitive files and without following out-of-root symlinks. Start with README files, routes, navigation, screens, feature folders, APIs, fixtures, demos, stories, tests, docs, and seed data. Preserve unrelated changes. **Complete when:** the inspected and skipped areas are recorded.
...
1. What the product appears to be and who it serves
2. Main modules, surfaces, or workspaces
3. Key jobs and actions users can perform
4. Day-to-day journeys and advanced paths
5. Roles, permissions, plans, configuration, and integrations
6. Mature, partial, unknown, and unverified areas
...
Produce one file at the approved repository root. Default to the self-contained `product-walkthrough.html`; produce `product-walkthrough.md` only when the user explicitly requests Markdown. Use these sections, adapted when evidence requires:

- `## What this product seems to be`
- `## Who it appears to serve`
- `## Product map`
- `## Core user journeys`
- `## Feature and capability inventory`
- `## Roles, configuration, and integrations`
- `## Evidence, maturity, and open questions`
...
1. The selected `product-walkthrough.html` or `product-walkthrough.md` is the only created or changed file and is inside the approved root.
