---
name: dx-domain-walkthrough
description: Use when a repository needs domain onboarding covering entities, terminology, rules, permissions, workflows, and lifecycle states; not for architecture alone.
license: Apache-2.0
...
Create an evidence-backed, start-to-finish repository walkthrough through the lens of business concepts and domain model. This is a teaching document, not a file inventory or unsupported critique.
...
Do not use for:
- Trace the application startup and request execution path.
- Review visual spacing and typography choices.
- List product features and user-facing modules without domain rules.
...
1. Confirm the repository root, requested audience, scope, excluded areas, format, and output path. Default to `domain-walkthrough.html`; use `domain-walkthrough.md` only when Markdown is explicitly requested. Check whether the selected output already exists. **Complete when:** boundaries, format, and overwrite status are explicit.
2. Build a bounded inventory without opening excluded or sensitive files and without following out-of-root symlinks. Start with schemas, models, migrations, API contracts, validation, state machines, policies, tests, fixtures, routes, and UI copy. Preserve unrelated changes. **Complete when:** the inspected and skipped areas are recorded.
...
1. The problem the system appears to model
2. Core entities, roles, and terminology
3. Relationships and bounded concept areas
4. Lifecycle states, transitions, and workflows
5. Rules, permissions, validations, and invariants
6. External systems, ambiguities, and unverified areas
...
Produce one file at the approved repository root. Default to the self-contained `domain-walkthrough.html`; produce `domain-walkthrough.md` only when the user explicitly requests Markdown. Use these sections, adapted when evidence requires:

- `## What this system seems to model`
- `## Core concepts and terminology`
- `## Entity and relationship map`
- `## Lifecycles and workflows`
- `## Rules, permissions, and invariants`
- `## External dependencies on the domain`
- `## Evidence, ambiguities, and open questions`
...
1. The selected `domain-walkthrough.html` or `domain-walkthrough.md` is the only created or changed file and is inside the approved root.
